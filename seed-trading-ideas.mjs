...
impimport * as schema from './drizzle/schema.js';
...
const [owner] = await db.select({ id: schema.users.id })
  .from(schema.users)
  .where(schema.users.openId.eq(process.env.OWNER_OPEN_ID))
  .limit(1);
...
await db.insert(schema.tradingIdeas).values(tradingIdeasData);
